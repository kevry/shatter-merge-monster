...
# Create a sub directory to keep track of edits made (splits)
...
# direct standard output to log file for debugging/tracking
        ...
        for ri in range(config["ROUNDS"]):
            ...
            while not l2_cache_success: 
                ...
                if not l2_cache_success:
                    time.sleep(config["WAIT_SEC"]) # wait to retry. l2 cache regeneration might take some time after doing a split operation
                
            # use l2 graph to to create clusters using the louvain algo
            clusters = create_cluster_groups(G, resolution=config["LOUVAIN_RESOLUTION"], verbose=True)

            # from the l2 graph, get the number of paths between each cluster
            ...
            clusters_main_information, modified_clusters = clusters_main_information, clusters
            next_pairs = []
            ...
            for p_index, pair in enumerate(next_pairs):
                ...
                if config["PREVIEW"]:
                    ...
                    try: #split
                        ...
                        # TODO: Make exception  LESS robust
                        #print(f"(ERROR) Split ERROR occurred: {str(e)}\n")
                        print(f"(ERROR) Split error caught. Logging and trying next", str(e))
                        time.sleep(2)
                        num_errors += 1

                if num_errors >= 3: # if reached error count thresh
                    MAX_ERROR_LIMIT_FOUND = True
                    break
            ...
            if MAX_ERROR_LIMIT_FOUND:
                if config["NUM_EDGES_PATHCUT"] == 1:   
                    config["NUM_EDGES_PATHCUT"] = 3 # start back at cutting paths with 3 neightbor clusters
                else:
                    config["NUM_EDGES_PATHCUT"] -= 1
                print("SWITICHING PATHCUT to ", config["NUM_EDGES_PATHCUT"])
                time.sleep(20)
                break
